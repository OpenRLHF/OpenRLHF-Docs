...
- You should increase the ``rollout_micro_batch_size`` (and minimize the TP size of vLLM engine) as much as possible, and avoid OOM (Out Of Memory) issues using ``--packing_samples``. During the training phase, a larger ``--micro_train_batch_size`` is better.
